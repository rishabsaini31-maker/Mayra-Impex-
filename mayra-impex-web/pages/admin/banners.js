...
export default function Banners() {
  ...
  return (
    <div style={{ marginLeft: 0, padding: 32, minHeight: "100vh", background: "#f9fafb" }}>
      <h2 style={{ textAlign: "center", fontWeight: 800, fontSize: 28, marginBottom: 24 }}>Banners Management</h2>
      ...
      {showAddModal && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "#0008", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 10, padding: 32, minWidth: 320, maxWidth: 400, boxShadow: "0 4px 24px #0002", position: "relative" }}>
            ...
            <h3 style={{ fontWeight: 700, fontSize: 22, marginBottom: 18, textAlign: "center" }}>Add Banner</h3>
      ...
      <div style={{ overflowX: "auto", background: "#fff", borderRadius: 8, boxShadow: "0 2px 8px #0001", padding: 16 }}>
        <table style={{ width: "100%", marginTop: 0, borderCollapse: "collapse", background: "#fff" }}>
          <thead>
            <tr>
              <th style={{ border: "1px solid #e5e7eb", padding: 12, background: "#f3f4f6" }}>ID</th>
              {/* Title column removed as requested */}
              <th style={{ border: "1px solid #e5e7eb", padding: 12, background: "#f3f4f6" }}>Image</th>
              <th style={{ border: "1px solid #e5e7eb", padding: 12, background: "#f3f4f6" }}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {banners.length === 0 ? (
              <tr>
                <td colSpan={4} style={{ textAlign: "center", color: "#888", padding: 24, border: "1px solid #e5e7eb" }}>No banners found.</td>
              </tr>
            ) : (
              banners.map((banner, idx) => (
                <tr key={banner.id} style={{ background: idx % 2 === 0 ? "#f9fafb" : "#fff" }}>
                  <td style={{ border: "1px solid #e5e7eb", padding: 10 }}>{banner.id}</td>
                  {/* Title cell removed as requested */}
                  <td style={{ border: "1px solid #e5e7eb", padding: 10 }}>
                    {banner.imageUrl ? <img src={banner.imageUrl} alt="banner" style={{ maxWidth: 80, maxHeight: 40, borderRadius: 4 }} /> : "-"}
                  </td>
                  <td style={{ border: "1px solid #e5e7eb", padding: 10 }}>
                    <button style={{ marginRight: 8, padding: "4px 12px", borderRadius: 4, background: "#2563eb", color: "#fff", border: "none", fontWeight: 600, cursor: "pointer" }} onClick={() => handleEditBanner(banner)}>Edit</button>
                    <button style={{ padding: "4px 12px", borderRadius: 4, background: "#dc2626", color: "#fff", border: "none", fontWeight: 600, cursor: "pointer" }} onClick={() => handleDeleteBanner(banner.id)}>Delete</button>
                  </td>
                </tr>
              ))
            )}
