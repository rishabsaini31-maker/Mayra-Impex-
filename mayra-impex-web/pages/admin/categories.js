...
export default function Categories() {
  ...
  return (
    ...
    >
      ...
      <div style={{ margin: "0 0 24px 0", display: "flex", gap: 16, alignItems: "center" }}>
        <button
        ...
        <input
          ...
          onChange={e => {
            ...
            setCategories((prev) => prev.map(cat => ({ ...cat, _hidden: cat.name.toLowerCase().indexOf(val) === -1 })))
          }}
        />
      </div>
      {/* Add Category Modal */}
      {showAddModal && (
        <div style={{
          ...
          <div style={{ background: "#fff", borderRadius: 10, padding: 32, minWidth: 320, maxWidth: 400, boxShadow: "0 4px 24px #0002", position: "relative" }}>
            ...
            <h3 style={{ fontWeight: 700, fontSize: 22, marginBottom: 18, textAlign: "center" }}>{addForm.id ? "Edit Category" : "Add Category"}</h3>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                setAddLoading(true);
        ...
        >
          ...
          <tbody>
            {categories.length === 0 ? (
              ...
              categories.filter(cat => !cat._hidden).map((category, idx) => (
                <tr key={category.id} style={{ background: idx % 2 === 0 ? "#f9fafb" : "#fff" }}>
                  <td style={{ border: "1px solid #e5e7eb", padding: 10 }}>{category.id}</td>
                  <td style={{ border: "1px solid #e5e7eb", padding: 10 }}>{category.name}</td>
                  <td style={{ border: "1px solid #e5e7eb", padding: 10 }}>
                    <button style={{ marginRight: 8, padding: "4px 12px", borderRadius: 4, background: "#2563eb", color: "#fff", border: "none", fontWeight: 600, cursor: "pointer" }} onClick={() => handleEditCategory(category)}>Edit</button>
                    <button style={{ padding: "4px 12px", borderRadius: 4, background: "#dc2626", color: "#fff", border: "none", fontWeight: 600, cursor: "pointer" }} onClick={() => handleDeleteCategory(category.id)}>Delete</button>
                  </td>
                </tr>
              ))
            )}
